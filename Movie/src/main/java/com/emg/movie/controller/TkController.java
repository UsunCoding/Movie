...
public class TkController {
	...
	public @ResponseBody List<reserveDTO> reserve(@ModelAttribute reserveDTO rsv){	
		List<reserveDTO> rsvList = tksvc.reserve(rsv);
		System.out.println("[controler:rsv] "+rsvList);
		return rsvList;	
	}	
	...
	public ModelAndView reView(@RequestParam("reserveId") int reserveId) {
		System.out.println("[1]jsp → controller" + reserveId);
	...
	public ModelAndView bDelete(@ModelAttribute reserveDTO rsv) {
		System.out.println("======삭제=======");
		System.out.println("[1]" + rsv);
		mav = tksvc.bDelete(rsv);
		System.out.println("[5]" + mav);
		return mav;
		// 상영정보 삭제
	}
